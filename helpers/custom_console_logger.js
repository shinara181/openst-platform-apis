...
var appendRequest = function(message) {
    var requestSharedNamespace = getNamespace('openST-Platform-APIs-NameSpace');
    var newMessage = "";
    if(requestSharedNamespace){
      if(requestSharedNamespace.get('reqId')){
        newMessage += ("[" + requestSharedNamespace.get('reqId') + "]");
      }
      if(requestSharedNamespace.get('workerId')){
        newMessage += ("[Worker - " + requestSharedNamespace.get('workerId') + "]");
      }
...
var calculateRequestTime = function() {
  var requestSharedNamespace = getNamespace('openST-Platform-APIs-NameSpace');
  var reqTime = 0;
  if(requestSharedNamespace && requestSharedNamespace.get('startTime')){
    var hrTime = process.hrtime();
    reqTime = timeInMilli(hrTime) - timeInMilli(requestSharedNamespace.get('startTime'));
  }
  return reqTime;
};
